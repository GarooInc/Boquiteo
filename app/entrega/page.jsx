...
const PageEntrega = () => {
  ...
  const promptConfirm = (action) => {
    ...
    const fetchData = async () => {
        const response = await fetch("https://boquiteo-garoo.koyeb.app/api/delivery/orders")
                ...
                <div className='px-8 columns-1 gap-5 sm:columns-2 sm:gap-8 md:columns-2 lg:columns-3 [&>div:not(:first-child)]:mt-8'>
                    {
                        filteredData.map((item) => {
                            return (
                                <div key={item._id} className='lg:w-full lg:m-0 mx-4 w-full bg-white p-4 rounded-md border-2 border-dark-gray flex flex-col items-start justify-start gap-2 overflow-y-auto h-auto'>
